...
class GeotCore {
	...
	public function __construct() {
		...
		if ( ! is_admin()
			...
			if (apply_filters('geot/enable_np_cookies', false ) ) {
				add_action( 'init', [ $this, 'createNitroPackCookies' ], 15 );
			}
		}
	...
	public function createNitroPackCookies() {
		$country = isset( $this->user_data[ $this->cache_key ]->country->iso_code ) ? $this->user_data[ $this->cache_key ]->country->iso_code : 'not detected';
		$state   = isset( $this->user_data[ $this->cache_key ]->state->iso_code ) ? $this->user_data[ $this->cache_key ]->state->iso_code : 'not detected';
		$city    = isset( $this->user_data[ $this->cache_key ]->city->name ) ? $this->user_data[ $this->cache_key ]->city->name : 'not detected';

		if( ! isset( $_COOKIE[$this->opts['cookie_name']] ) ) {
			setcookie( 'geot_country', apply_filters( 'geot_country', $country ), 0, '/', '', true );
			setcookie( 'geot_state', apply_filters( 'geot_state', $state ), 0, '/', '', true );
			setcookie( 'geot_city', apply_filters( 'geot_city', $city ), 0, '/', '', true );
		}
	}
